							...
							<?foreach($arItem["FIELDS"] as $code=>$value):?>
								...
								<?elseif($code == "TAGS" && $value):?>
									<div class="news-list-view news-list-post-params">
																													<?$arr=explode(",",$value);
foreach($arr as $code):
		$code=trim($code);  switch ($code) {
    case "Наука":?>
    	<span class="news-list-value" style="color: #fff; background: #5fa8ed;">
        <?break;
 

   case "Абитуриентам":?>
        <span class="news-list-value" style="color: #ffa; background: #faae10;">
        	  <?break;
...
class="news-list-value" style="color: #ffa; background: #faae10;">
        	
<?break;}?><?=$code;?></span><?endforeach;?>
